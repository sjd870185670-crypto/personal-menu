...
// 根据每日热量差估算每周体重变化（斤）
// 7700 kcal ≈ 1kg 脂肪 ≈ 2 斤
export function estimateWeightJinPerWeek(dailyDeltaKcal) {
  if (!dailyDeltaKcal || dailyDeltaKcal === 0) return 0;
  const kgPerWeek = (dailyDeltaKcal * 7) / 7700;
  return Math.round(kgPerWeek * 2 * 100) / 100;
}
